...
const Layout: React.FC<LayoutProps> = ({ 
  ...
  const handleSearchSubmit = (e: React.FormEvent) => {
    ...
    onChangeView('home'); // Switch to home to see results
  ...
  return (
    <div className="min-h-screen flex flex-col bg-gray-50">
      {/* Navbar (Sticky Top) */}
      <nav className="sticky top-0 z-40 bg-white h-16 border-b border-gray-200 px-4 flex items-center justify-between shadow-sm relative">
        <div className="flex items-center gap-4">
          <div className="flex items-center gap-2 cursor-pointer" onClick={() => { onChangeView('home'); onCategoryChange(WorkerCategory.ALL); setSearchQuery(""); onSearch(""); }}>
        ...
        <div className="flex items-center gap-3">
           ...
           <button 
             className={`text-gray-600 p-2 hover:bg-gray-100 rounded-full md:hidden transition-colors ${isMobileSearchOpen ? 'bg-gray-100 text-green-600' : ''}`} 
      ...
      <div className={`md:hidden bg-white border-b border-gray-200 overflow-hidden transition-all duration-300 ease-in-out z-30 ${isMobileSearchOpen ? 'max-h-20 opacity-100' : 'max-h-0 opacity-0'}`}>
        <form onSubmit={handleSearchSubmit} className="p-3">
          <div className="relative">
             ...
             />
             <button type="submit" className="absolute right-0 top-0 h-full px-4 text-green-600 hover:text-green-800">
                <i className="fas fa-search"></i>
             </button>
          </div>
        </form>
